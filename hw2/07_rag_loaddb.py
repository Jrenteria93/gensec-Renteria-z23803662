...
# from langchain_community.document_loaders import AsyncHtmlLoader, DirectoryLoader, TextLoader, PyPDFLoader, Docx2txtLoader, UnstructuredMarkdownLoader, WikipediaLoader, ArxivLoader, CSVLoader, GithubFileLoader
from langchain_community.document_loaders import AsyncHtmlLoader, DirectoryLoader, TextLoader, PyPDFLoader, Docx2txtLoader, UnstructuredMarkdownLoader, ArxivLoader, CSVLoader, GithubFileLoader
...
def load_pdf(directory):
    ...
    Uses LangChain's PyPDFLoader (via DirectoryLoader) the same way load_txt
    uses TextLoader, so both the original .txt file type and PDFs can be
    ingested with the same modular pattern.
    """
    load_docs(DirectoryLoader(directory, glob="**/*.pdf", loader_cls=PyPDFLoader).load())
